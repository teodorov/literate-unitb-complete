...
module Language.UnitB.Parser.Tests.LockFreeDeque 
    -- ( test_case, test, path4, result4 )
where

    -- Modules
import Language.UnitB.Parser.Tests.Suite as S
import Language.UnitB.Parser.Tests.LockFreeDeque.Stable
...
test = test_cases
            ...
            , (textCase "test 2, error message name clash in guards" 
                case2 result2)
            ...
            , (textCase "test 5, transient, two branches, enablement of non-empty"
                (proof_obligation path4 "m1/LIVE/m1:prog3/ensure/TR/m0:pop:left:non:empty/EN" 1)
                result5)
            , (textCase "test 6, transient, two branches, enablement of empty"
                (proof_obligation path4 "m1/LIVE/m1:prog3/ensure/TR/m0:pop:left:empty/EN" 1)
                result6)
            , (textCase "test 7, transient, two branches, negation of empty"
                (proof_obligation path4 "m1/LIVE/m1:prog3/ensure/TR/m0:pop:left:empty/NEG" 1)
                result7)
            , (textCase "test 8, transient, two branches, negation of non-empty"
                (proof_obligation path4 "m1/LIVE/m1:prog3/ensure/TR/m0:pop:left:non:empty/NEG" 1)
                result8)
            , (textCase "test 9, transient, two branches, follow and disjunction"
                (proof_obligation path4 "m1/LIVE/m1:prog3/ensure/TR/leadsto" 1)
                result9)
            , (textCase "test 10, duplicate refinement of liveness properties"
                (find_errors path10)
                result10)
            , textCase "test 11, missing witness"
                (find_errors path11)
                result11
            ...
            , aCase "test 16, event splitting, index decl"
                case16 result16
            , textCase "test 17, empty parameter list"
                case17 result17
            , textCase "test 18, empty list in VarScope"
                case18 result18
            , textCase "test 19, splitting POs"
                case19 result19
            ...
            --     case27 result27
            , textCase "test 28, large WD-condition"
                case28 result28
            , textCase "test 29, weakening and splitting / disjunction"
                case29 result29
            , textCase "test 30, weakening and splitting / safety"
                case30 result30
            , textCase "test 31, INIT/fis"
                case31 result31
            , textCase "test 32, inferring used record types"
                case32 result32
            , poCase "test 33, verifying with inferred records"
                case33 result33
            , textCase "test 34, using coarse schedules in proofs of safety of new events"
                case34 result34
            , poCase "test 35, weakened coarse schedule replacement leads-to"
                case35 result35
            , aCase "test 36, deleting indices - AST"
                case36 result36
            , textCase "test 37, deleting indices - PO"
                case37 result37
            , poCase "test 38, deleting indices - verify"
                case38 result38
            , textCase "test 39, deleting indices - Wit FIS"
                case39 result39
            ...
            , poCase "test 42, use new schedules in proofs of weakening"
                case42 result42
            , textCase "test 43, use new schedules in proofs of weakening, PO"
                case43 result43
            -- , textCase "test 44, weird error with merging"
            --     case44 result44
            , textCase "test 45, weird error with merging"
                case45 result45
            , textCase "test 46, inconsistent merge event"
                case46 result46
            , textCase "test 47, deleting indices of non-existant event"
                case47 result47
            , textCase "test 48, invariant violation upon index deletion"
                case48 result48
            , textCase "test 49, guard strengthening / merging with index deletion"
                case49 result49
            , textCase "test 50, improper error message"
                case50 result50
            , textCase "test 51, invalid goal for liveness proof"
                case51 result51
            , textCase "test 52, deleted variables are not primed"
                case52 result52
            , textCase "test 53, bad rendering of expressions"
                case53 result53
            ]            
...
result14 :: Either Text ([SkipOrEvent],[SkipOrEvent],[SkipOrEvent])
...
case15 :: IO (Either Text (ExprSet,ExprSet,ExprSet))
...
result15 :: Either Text (ExprSet,ExprSet,ExprSet)
result15 = Right   ( [(Left SkipEvent,[]),("evt",["act0"])]
                   , [(Left SkipEvent,[]),("evt0",["act0"]),("evt1",["act0","grd0"]),("evt2",["act0"])]
                   , [(Left SkipEvent,[]),("evt0",["act0"]),("evt1",["act0","grd0"]),("evt2",["act0"])])

case16 :: IO (Either Text (ExprSet,ExprSet,ExprSet))
...
result16 :: Either Text (ExprSet,ExprSet,ExprSet)
