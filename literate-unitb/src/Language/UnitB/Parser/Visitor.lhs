...
module Language.UnitB.Parser.Visitor 
    ...
    , M (..)
    , unM, runM
    , Language.UnitB.Parser.Visitor.hoistEither
    , hoistValidation
    , triggerV
    , unfailV
    , Language.UnitB.Parser.Visitor.left
    , AllReadable (..) )
where
...
visitor blks cmds = do
        unless (all (\(x,_) -> "\\" `T.isPrefixOf` x) cmds) 
            $ error $ unpack $ "Language.UnitB.Parser.Visitor.visitor: "
...
--        s0 <- RWS.get
        unless (all (\(x,_) -> "\\" `T.isPrefixOf` x) cmds) 
            $ error $ unpack $ "Language.UnitB.Parser.Visitor.visit_doc: "
                    <> [st|all commands must start with '\\': %s|] (show $ map fst cmds)
