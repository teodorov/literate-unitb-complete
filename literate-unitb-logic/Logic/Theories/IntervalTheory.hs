...
interval_theory = (empty_theory' "intervals") 
        ...
        , _fact = M.mapKeys label $ M.fromList
            [ ("axm0", fromRight' $ mzforall [x_decl,m_decl,n_decl] mztrue $
                                (x `zelem` zinterval m n) 
                        `mzeq`  (zbetween mzle mzle m x n))
            , ("axm1", fromRight' $ mzforall [x_decl,m_decl,n_decl] mztrue $
                                (x `zelem` zinterval_r m n) 
                        `mzeq`  (zbetween mzle mzless m x n))
            , ("axm2", fromRight' $ mzforall [x_decl,m_decl,n_decl] mztrue $
                                (x `zelem` zinterval_l m n) 
                        `mzeq`  (zbetween mzless mzle m x n))
            , ("axm3", fromRight' $ mzforall [x_decl,m_decl,n_decl] mztrue $
                                (x `zelem` zinterval_l m n) 
                        `mzeq`  (zbetween mzless mzle m x n))
            , ("axm4", fromRight' $ mzforall [m_decl] mztrue $
                                zinterval_r m m
                        `mzeq`  zempty_set)
            , ("axm5", fromRight' $ mzforall [m_decl] mztrue $
                                zinterval_l m m
                        `mzeq`  zempty_set)
            , ("axm6", fromRight' $ mzforall [m_decl] mztrue $
                                zinterval_r m (m + z1)
                        `mzeq`  zmk_set m)
            , ("axm7", fromRight' $ mzforall [m_decl] mztrue $
                                zinterval_l (m - z1) m
                        `mzeq`  zmk_set m)
            , ("axm8", fromRight' $ mzforall [m_decl,n_decl,p_decl] 
                            (zbetween mzle mzle m n p) $
                                (zinterval_r m n `zunion` zinterval_r n p)
                        `mzeq`  (zinterval_r m p))
            , ("axm9", fromRight' $ mzforall [m_decl,n_decl,p_decl] 
                            (zbetween mzle mzle m n p) $
                                (zinterval_l m n `zunion` zinterval_l n p)
                        `mzeq`  (zinterval_l m p))
            , ("axm10", fromRight' $ mzforall [m_decl,n_decl] 
                            (mzle m n) $
                                (zinterval_r m n `zunion` zmk_set n)
                        `mzeq`  (zinterval_r m (n+z1)))
            , ("axm11", fromRight' $ mzforall [m_decl,n_decl] 
                            (mzle m n) $
                                (zinterval_l m n `zunion` zmk_set (n + z1))
                        `mzeq`  (zinterval_l m (n+z1)))
            , ("axm12", fromRight' $ mzforall [m_decl,n_decl] 
                            (mzle m n) $
                                (zmk_set (m - z1) `zunion` zinterval_r m n)
                        `mzeq`  (zinterval_r (m - z1) n))
            , ("axm13", fromRight' $ mzforall [m_decl,n_decl] 
                            (mzle m n) $
